...
const scoreHex = (s) => (s >= 7 ? '22C55E' : s >= 4 ? 'F59E0B' : 'EF4444');
...
export async function downloadReport(evaluation, overrides = {}, role = 'student') {
  ...
  children.push(
    new Paragraph({
      ...
      spacing: { after: 40 },
      children: [
        new TextRun({ text: 'ClarifAI', size: 44, bold: true, color: BRAND }),
  ...
  children.push(
    labelValue('Overall Score', `${overall_score} / 10`),
  ...
  const scoreRows = scores.map((s) => {
    ...
    const rating = (sc) => (sc >= 7 ? 'Excellent' : sc >= 4 ? 'Satisfactory' : 'Needs Improvement');

    if (role === 'instructor') {
      return new TableRow({
        children: [
          dataCell(s.criterion),
          dataCell(`${s.score}/${s.max || 10}`, { center: true, color: scoreHex(s.score), bold: true }),
          dataCell(
            hasOverride ? `${instrScore}/${s.max || 10}` : '—',
            { center: true, color: hasOverride ? scoreHex(instrScore) : '999999', bold: hasOverride },
          ),
    ...
    return new TableRow({
      children: [
        dataCell(s.criterion),
        dataCell(`${s.score}/${s.max || 10}`, { center: true, color: scoreHex(s.score), bold: true }),
        dataCell(rating(s.score), { center: true }),
      ],
    });
  });
      ...
      children.push(
        new Paragraph({
          ...
          children: [
            ...
            new TextRun({ text: `  (${e.score}/10 — ${e.sentiment || 'N/A'})`, size: 20, color: '888888' }),
          ],
        }),
    ...
    const cfRows = counterfactuals.map((cf) =>
      new TableRow({
        children: [
          ...
          dataCell(`${cf.current_score}/10`, { center: true }),
          dataCell(`${cf.predicted_new_score}/10`, { center: true, color: '22C55E', bold: true }),
  ...
  children.push(
    new Paragraph({
      ...
      children: [
        new TextRun({
          text: 'Powered by ClarifAI — Explainable AI Grading',
          size: 18,
          color: 'AAAAAA',
          italics: true,
        }),
  ...
  const doc = new Document({
    creator: 'ClarifAI',
    title: 'ClarifAI Grade Evaluation Report',
    description: 'AI-generated evaluation report with evidence and coaching',
    sections: [{ children }],
  });

  const blob = await Packer.toBlob(doc);
  const fileName = `ClarifAI_Report_${role}_${now.toISOString().slice(0, 10)}.docx`;
