...
Your job is to score EACH rubric criterion on a scale of 1 to 10.

SCORING SCALE (1–10):
  9–10 : Excellent — exceeds expectations, demonstrates mastery
  7–8  : Good — meets expectations with minor gaps
  5–6  : Satisfactory — partially meets expectations
  3–4  : Needs Improvement — significant gaps present
  1–2  : Poor — criterion barely addressed or missing
...
For each criterion where score < 8, generate a counterfactual:
...
- Predicted new score must be realistic (do not promise 10/10 for minor changes).
- Skip criteria where score >= 8 (already good, no counterfactual needed).
- Your response must be ONLY valid JSON, no extra text, no markdown.
